...
8. **🗂 SSOT** — 见下方"🗂 SSOT"
9. **💰 P&L 术语** — 见下方"💰 P&L 术语"
10. **🚫 P1-P3.6 全程禁用 AskUserQuestion** — 报告内写完整推荐 + 利弊对比 + 推荐动作,用户读完报告自行决定。唯一例外:P3.7 audit 发现致命数据冲突(如持仓量与用户报告矛盾)。**反模式**:中途询问"成交吗?"/"选哪个场景?"。**正确**:报告里写"假设未挂单 / 待用户确认成交;推荐 X 因 Y"。详见 [[feedback_briefing_no_questions]]
11. **🕐 跨午休 / 跨收盘必先重拉** — 用户任何 follow-up 请求,若上次拉数距当前 BJT 跨越 11:30-13:00 或 15:00 分界,**首动作必 Bash date + 重拉持仓+指数**,不需询问
12. **📋 "今天建议"必出全持仓+全关注清单扫描表** — 用户问"今天什么建议 / 今天注意什么 / 今天怎么做"时,**禁止只列 3-5 个接近触发的标的**。必输出 4 块:① 完整持仓表(N 行 = 持仓数,每行写"动作+细节") ② 完整关注清单表(positions.md 关注清单全部标的,即使"不动"也写一行说明原因) ③ 板块/事件汇总 ④ 最终决策清单(🔴必做/🟡监控/🟢不动)。**用户没时间读 report,扫描表替代 report**。详见 [[feedback_full_scan_table]]
...
## 催化剂预案(本周重大事件 3 场景)
- **事件**:[NVDA 财报 / Fed FOMC / CPI 发布等]
- **场景表**:🟢 Beat / 🟡 Inline / 🔴 Miss × 概率% × 我方动作(具体股数/挂单价)
- *(本段独立于"操作建议",专门放外部事件应急预案)*
