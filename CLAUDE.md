...
### 反模式(严禁)
- ❌ "之前透支 ¥1,145 → 现在应该是 ¥X" (这是基于过期数据推算)
- ❌ "估算占比约 57%" (没算就别给数字)
- ❌ "可能在 ¥10-15 之间" (区间猜测无意义)
- ❌ "黄金大概值 ¥83K" (汇率/单价/纯度全在变)

### 正确做法
- ✅ "用户确认现金 ¥16,600"
- ✅ "Yahoo 5/11 12:39 BJT 太极 ¥11.24 marketState=REGULAR"
- ✅ "我不确定,你能确认一下 X 吗?"

### 触发条件
- 任何用户决策依赖的数字 → 必须实测 / 确认
- 任何写入 positions.md / decisions/ / reports/ 的数字 → 必须实测 / 确认
- 计算仓位占比 / 浮盈 / 总市值 → 显式列出每个分子分母,不省略
